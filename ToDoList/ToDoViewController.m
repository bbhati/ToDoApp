...
//    ToDoCell* cellContainingFirstResponder = (ToDoCell*)[textView superViewWithClass:[UITableViewCell class]];
    ToDoCell* cellContainingFirstResponder = objc_getAssociatedObject(textView, "cell");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.row < [self.toDoItems count]){
        ...
        if(cell != nil){
            ...
            objc_setAssociatedObject(cell.todoTextView, "cell", cell, OBJC_ASSOCIATION_RETAIN);
        }
        
        return cell;
    }
    else if(indexPath.row == [self.toDoItems count]){
        static NSString *CellIdentifier = @"DummyCell";
        DummyCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier forIndexPath:indexPath];
        return cell;
    }
    else return nil;
}
...
- (void)keyboardWasShown:(NSNotification*)aNotification
{
    ...
    NSLog(@"keyboard width: %f", kbSize.width);
    ...
    if (UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation))
    {
        ...
        if (!CGRectContainsPoint(aRect, self.activeView.frame.origin) ) {
            [self.tableView scrollRectToVisible:self.activeView.frame animated:YES];
        }
    }
